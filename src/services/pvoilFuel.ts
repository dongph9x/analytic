...
export interface PvoilFuelResult {
  ron95: number | null;
  do: number | null;
  tableText: string;
}

export async function fetchPvoilFuelTable(): Promise<PvoilFuelResult> {
  const result: PvoilFuelResult = { ron95: null, do: null, tableText: '' };

  try {
    const res = await axios.get<string>(PVOIL_URL, {
      timeout: 15000,
      headers: {
    ...
    console.error('PVOIL fetch error:', (err as Error).message);
  }
